@section('secciones')
 <br> 

<section class="py-5">
	<div class="container">
        
		<div class="row justify-content-center text-center mb-1 ">
			<div class="col-lg-8 col-xxl-7">
				<h2 class="display-5 fw-bold ">Como comprar un dominio?</h2>
				<p class="lead">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Soluta harum ipsum venenatis metus sem veniam eveniet aperiam suscipit.</p>
			</div>
		</div>
        <br>
        <br>
		<div class="row">
			<div class="col-md-3 ">
				<div class="text-center position-relative">
					<div class="step-icon mx-auto bg-primary border rounded-circle d-flex align-items-center justify-content-center " style="width:120px;height:120px;">
						<img
                    src="/img/imgPruebas/iconos_secciones/compras.png"
                    height="50"
                    alt="Logo"
                    loading="lazy"
                />
					</div>
					<h4 class="mt-3 fs-5">Realizar la compra</h4>
					<p class="lead text-muted mt-4 fs-5 px-lg-3 mb-5 mb-lg-0">Lorem ipsum dolor sit ame consectetur Lorem.</p>
					<div class="arrow-icon position-absolute d-none d-lg-block" style="top:50px; right:-25px">
						<img
                    src="/img/imgPruebas/iconos_secciones/flecha.png"
                    height="50"
                    alt="Logo"
                    loading="lazy"
                />
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="text-center position-relative">
					<div class="step-icon mx-auto bg-primary border rounded-circle d-flex align-items-center justify-content-center" style="width: 120px;height: 120px;">
						<img
                    src="/img/imgPruebas/iconos_secciones/email.png"
                    height="50"
                    alt="Logo"
                    loading="lazy"
                />
					</div>
					<h4 class="mt-3 fs-5">Envio al Email</h4>
					<p class="lead text-muted mt-4 fs-5 px-lg-3 mb-5 mb-lg-0">Lorem ipsum dolor sit ame consectetur Lorem.</p>
					<div class="arrow-icon d-none d-lg-block position-absolute" style="top:50px; right:-25px">
						<img
                    src="/img/imgPruebas/iconos_secciones/flecha.png"
                    height="50"
                    alt="Logo"
                    loading="lazy"
                />
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="text-center position-relative">
					<div class="step-icon mx-auto bg-primary border rounded-circle d-flex align-items-center justify-content-center" style="width: 120px;height: 120px;">
						<img
                    src="/img/imgPruebas/iconos_secciones/entrega.png"
                    height="50"
                    alt="Logo"
                    loading="lazy"
                />
					</div>
					<h4 class="mt-3 fs-5">Envio del equipo</h4>
					<p class="lead text-muted mt-4 fs-5 px-lg-3 mb-5 mb-lg-0">Lorem ipsum dolor sit ame consectetur Lorem.</p>
					<div class="arrow-icon d-none d-lg-block position-absolute" style="top:50px; right:-25px">
						<img
                    src="/img/imgPruebas/iconos_secciones/flecha.png"
                    height="50"
                    alt="Logo"
                    loading="lazy"
                />
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="text-center position-relative">
					<div class="step-icon mx-auto bg-primary border rounded-circle d-flex align-items-center justify-content-center" style="width: 120px;height: 120px;">
						<img
                    src="/img/imgPruebas/iconos_secciones/qr.png"
                    height="50"
                    alt="Logo"
                    loading="lazy"
                />
					</div>
					<h4 class="mt-3 fs-5">Activar el producto</h4>
					<p class="lead text-muted mt-4 fs-5 px-lg-3 mb-5 mb-lg-0">Lorem ipsum dolor sit ame consectetur Lorem.</p>
				</div>
			</div>
		</div>
	</div>
</section>
@endsection